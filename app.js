/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   CONFIGURACIÃ“N â€” Reemplazar con valores reales
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
const CONFIG = {
  // â”€â”€â”€ sb â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  supabaseUrl:   'https://avwqmeberbjadrrlvaaa.supabase.co',
  supabaseKey:   'sb_publishable_McWl7xNoHVDbUdaR51OFew_TdJTJw30',

  // â”€â”€â”€ MERCADOPAGO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // Si mpPublicKey estÃ¡ vacÃ­o, el pago se simula (modo demo)
  ...
  return {
    ...
    functions: {
      async invoke(name, { body } = {}) {
        try {
          ...
          return {
            data: null,
            error: { message: error?.message || 'No se pudo conectar con la funciÃ³n' }
          };
        }
      }
    },
    auth: {
      ...
      async signInWithOAuth() {
        return { error: { message: 'Google OAuth no estÃ¡ habilitado en esta versiÃ³n estÃ¡tica.' } };
      },
...
const CHART_DATA = [
  {day:'Lun',val:8500},{day:'Mar',val:12000},{day:'MiÃ©',val:5000},{day:'Jue',val:18500},
  {day:'Vie',val:22000},{day:'SÃ¡b',val:9000},{day:'Dom',val:14500},
];
...
function renderFormHeader() {
  ...
  if (metaNode) metaNode.textContent = `${role} Â· ${city}`;
    ...
    nr.innerHTML = `
      ...
      <div class="nav-dropdown-wrap" id="ddWrap">
        ...
        <div class="nav-dropdown" id="navDD">
          <button class="dd-item" onclick="nav('profile');closeDD()">ðŸ‘¤ Ver mi perfil</button>
          <button class="dd-item" onclick="nav('settings');closeDD()">âš™ï¸ ConfiguraciÃ³n</button>
          <div class="dd-sep"></div>
          <button class="dd-item danger" onclick="doLogout()">â†© Cerrar sesiÃ³n</button>
        </div>
      </div>`;
  } else {
    nr.innerHTML = `
...
async function renderMpBrick() {
  ...
  if (!hasCredentials) {
    // Modo demo: sin brick, el botÃ³n simula
    brickContainer.innerHTML = `
...
async function createMpPreference() {
  ...
  if (!amount || amount < 100) { toast('El monto mÃ­nimo es $100','error'); return null; }

  // En producciÃ³n: llamar a la Edge Function de sb
  if (sb) {
    try {
      const { data, error } = await sb.functions.invoke('mp-create-preference', {
        body: {
          ...
          reviewerName: STATE.isAnon ? 'AnÃ³nimo' : ((document.getElementById('fNombre')?.value || '') + ' ' + (document.getElementById('fApellido')?.value || '')).trim(),
...
async function doLogin() {
  ...
  if (!email || !pw) { toast('CompletÃ¡ todos los campos','error'); return; }
  ...
  if (sb) {
    try {
      ...
      toast('Â¡Bienvenido de nuevo! ðŸ‘‹','success');
      setTimeout(() => nav('dashboard'), 600);
    } catch(e) { toast('Error al iniciar sesiÃ³n','error'); }
  } else {
    // Demo
    STATE.loggedIn = true;
    toast('Â¡Bienvenida de nuevo, Marcela! ðŸ‘‹','success');
    setTimeout(() => nav('dashboard'), 600);
  }
}
...
async function doRegister() {
  ...
  if (!nombre || !email || !pw) { toast('CompletÃ¡ todos los campos','error'); return; }
  if (pw.length < 8) { toast('La contraseÃ±a debe tener al menos 8 caracteres','error'); return; }
  ...
  if (sb) {
    try {
      ...
      if (data.session) {
        await hydrateUser(data.session, false);
        toast('Ã‚Â¡Perfil creado exitosamente! Ã¢Å“Â¦','success');
        setTimeout(() => nav('dashboard'), 600);
        return;
      }
      toast('Cuenta creada. RevisÃƒÂ¡ tu email para confirmar el acceso.','success');
      ...
      toast('Â¡Perfil creado exitosamente! âœ¦','success');
      setTimeout(() => nav('dashboard'), 600);
    } catch(e) { toast('Error al crear cuenta','error'); }
  } else {
    STATE.loggedIn = true;
    STATE.user.name = nombre;
    toast('Â¡Perfil creado! âœ¦','success');
    setTimeout(() => nav('dashboard'), 600);
  }
}
  ...
  if (method === 'mercadopago') {
    ...
    if (transferBox) {
      if (method === 'transfer') {
        ...
        if (transferText) transferText.textContent = `Alias: ${alias} Â· CBU/CVU: ${cbu}`;
      } else {
  ...
  list.innerHTML = STATE.rewardItems.map(item => `
    <div class="gallery-admin-card">
      ...
      <div>
        ...
        <div class="gallery-admin-meta">${item.description || 'Sin descripciÃ³n'}<br>${item.active ? 'Activa y visible cuando corresponda' : 'Oculta'}</div>
      </div>
    ...
    <div class="gallery-card">
      ...
      <div class="gallery-card-body">
        ...
        <div class="gallery-cta">
          <button class="btn btn-amber btn-sm" onclick="nav('form')">Quiero dejar mi reseÃ±a</button>
        </div>
      </div>
    </div>`).join('');
}
    ...
    return `
      <div class="gallery-card">
        <div class="gallery-media" style="${bgStyle}">
          ${overlay}
          <div class="gallery-badge-row">
            <span class="gallery-pill">${item.isCombo ? `Combo Â· ${mediaKindLabel(item.mediaKind)}` : mediaKindLabel(item.mediaKind)}</span>
            <span class="gallery-pill">${locked ? 'Bloqueada' : (item.visibility === 'public' ? 'PÃºblica' : 'Desbloqueada')}</span>
          </div>
        </div>
        <div class="gallery-card-body">
          <div class="gallery-title-row">
          ...
          <div class="gallery-desc">${item.description || 'Contenido disponible en la galerÃ­a.'}</div>
...
async function saveRewardItem() {
  if (!sb || !STATE.user.id) return toast('IniciÃ¡ sesiÃ³n para guardar recompensas','error');
  const title = document.getElementById('rewardTitle')?.value?.trim() || '';
  if (!title) return toast('EscribÃ­ un tÃ­tulo para la recompensa','error');
  ...
  renderPublicRewards();
  resetRewardForm();
  toast('Recompensa guardada âœ“','success');
}
...
async function deleteRewardItem(id) {
  ...
  renderPublicRewards();
  resetRewardForm();
  toast('Recompensa eliminada âœ“','success');
}
...
function renderMediaAdmin() {
  ...
  if (!STATE.mediaItems.length) {
    list.innerHTML = '<div class="gallery-admin-empty">TodavÃ­a no cargaste imÃ¡genes o combos.</div>';
    return;
  }
  list.innerHTML = STATE.mediaItems.map(item => `
    <div class="gallery-admin-card">
      ...
      <div>
        <div class="gallery-admin-title">${item.title}</div>
        <div class="gallery-admin-meta">${item.description || 'Sin descripciÃ³n'}<br>${galleryPriceLabel(item.priceCents)} Â· ${item.isCombo ? 'Combo' : mediaKindLabel(item.mediaKind)} Â· ${item.visibility === 'public' ? 'PÃºblica' : 'Privada'} Â· ${item.allowDownload ? 'Descargable' : 'Solo visualizaciÃ³n'} Â· ${item.active ? 'Activa' : 'Oculta'}</div>
      </div>
      <div class="gallery-admin-actions">
...
async function saveMediaItem() {
  if (!sb || !STATE.user.id) return toast('IniciÃ¡ sesiÃ³n para guardar imÃ¡genes','error');
  const title = document.getElementById('mediaTitle')?.value?.trim() || '';
  if (!title) return toast('EscribÃ­ un tÃ­tulo para la imagen','error');
  ...
  renderMediaVault();
  resetMediaForm();
  toast('Imagen guardada âœ“','success');
}
...
async function deleteMediaItem(id) {
  ...
  renderMediaVault();
  resetMediaForm();
  toast('Imagen eliminada âœ“','success');
}
...
async function purchaseMediaItem(mediaItemId) {
  ...
  if (!sb) {
    ...
    renderMediaVault();
    toast('Contenido desbloqueado en modo demo âœ“','success');
    return;
  }
...
async function submitReview() {
  ...
  // Si hay MP configurado â†’ crear preferencia y redirigir
  const pk = getMpPublicKey();
  if (STATE.selectedPay === 'mercadopago') {
    const pref = await createMpPreference();
    if (!pref) { btn.disabled = false; btnTxt.textContent = 'âœ¦ Pagar y publicar reseÃ±a'; return; }

    // Guardar datos temporales para mostrar en confirmaciÃ³n post-pago
    sessionStorage.setItem('aplauso_pending', JSON.stringify({
      reviewId: pref.review_id || '',
      profileSlug: STATE.viewedProfile?.slug || STATE.user.slug || '',
      nombre: STATE.isAnon ? 'AnÃ³nimo' : (nombre + ' ' + apellido).trim(),
      pay: 'ðŸ’³ MercadoPago',
      amount: STATE.selectedAmt,
      msg
    }));

    // En producciÃ³n redirige al checkout de MP
  ...
  if (sb) {
    try {
      ...
      document.getElementById('csPago').textContent = STATE.selectedPay === 'mercadopago' ? 'ðŸ’³ MercadoPago' : STATE.selectedPay === 'transfer' ? 'ðŸ¦ Transferencia' : 'ðŸ’° Efectivo';
      document.getElementById('csAmount').textContent = '$' + STATE.selectedAmt.toLocaleString('es-AR') + ' âœ¦';
      document.getElementById('fMsg').value = '';
      btn.disabled = false;
      btnTxt.textContent = 'âœ¦ Pagar y publicar reseÃ±a';
      toast(data?.published ? 'ReseÃ±a guardada en la base âœ“' : 'ReseÃ±a guardada pendiente de pago âœ“','success');
      renderProfile();
      nav('confirm');
      return;
    } catch(e) {
  ...
  btn.disabled = false;
  btnTxt.textContent = 'âœ¦ Pagar y publicar reseÃ±a';
  toast('ReseÃ±a publicada exitosamente âœ“','success');
  nav('confirm');
}
...
function renderProfile() {
  ...
  document.getElementById('pubQuickbar').innerHTML = `
    <div class="pub-quickbar-copy">
      <strong>Dejar una reseÃ±a lleva menos de 1 minuto.</strong>
      <span>EscribÃ­s, elegÃ­s el monto y publicÃ¡s.</span>
    </div>
    <div class="pub-quickbar-actions">
      <button class="btn btn-amber btn-sm" onclick="nav('form')">Dejar reseÃ±a</button>
  ...
  document.getElementById('pubReviewHero').innerHTML = `
    <div class="pub-review-spotlight">
      <div class="eyebrow" style="margin-bottom:12px">Tu reseÃ±a es la protagonista</div>
      <h2>Tu reseÃ±a ayuda a que otras personas confÃ­en.</h2>
      <p>ReconocÃ© una buena experiencia de forma simple, clara y rÃ¡pida.</p>
      <div class="pub-review-points">
        <div class="pub-review-point"><strong>RÃ¡pido</strong><span>Se completa en pocos pasos.</span></div>
        <div class="pub-review-point"><strong>Seguro</strong><span>Pago con Mercado Pago.</span></div>
        <div class="pub-review-point"><strong>Ãštil</strong><span>Le da mÃ¡s valor a la recomendaciÃ³n.</span></div>
      </div>
    </div>
    <div class="pub-review-card">
      <div>
        <div class="pub-review-kicker">AcciÃ³n principal</div>
        <h3>DejÃ¡ tu reseÃ±a ahora.</h3>
        <p>ElegÃ­ un monto, escribÃ­ unas lÃ­neas y listo.</p>
        <div class="pub-review-mini-proof">
          <div><strong>Menos de 1 minuto.</strong></div>
          <div><strong>AnÃ³nimo opcional.</strong></div>
          <div><strong>PublicaciÃ³n automÃ¡tica.</strong></div>
        </div>
      </div>
      <div class="pub-review-actions">
        <button class="btn btn-amber btn-md" onclick="nav('form')">Dejar reseÃ±a ahora</button>
        <button class="btn btn-surface btn-md" onclick="document.getElementById('pubReviews').scrollIntoView({ behavior: 'smooth', block: 'start' })">Ver reseÃ±as</button>
      </div>
    </div>`;
  document.getElementById('pubProofStrip').innerHTML = `
    <div class="pub-proof-card"><div class="pub-proof-kicker">ReseÃ±as</div><div class="pub-proof-value">${reviews.length}</div><div class="pub-proof-note">Publicadas</div></div>
    <div class="pub-proof-card"><div class="pub-proof-kicker">Promedio</div><div class="pub-proof-value">${formatCurrency(avg || 0)}</div><div class="pub-proof-note">Por reseÃ±a</div></div>
  ...
  document.getElementById('pubCredGrid').innerHTML = `
    <div class="pub-cred-card">
      <h3>Por quÃ© una reseÃ±a con recompensa da mÃ¡s credibilidad</h3>
      <p>Porque muestra una experiencia real y un reconocimiento genuino. Eso genera mÃ¡s confianza y tambiÃ©n motiva a seguir haciendo un buen trabajo.</p>
      <div class="pub-cred-points">
        <div class="pub-cred-point"><strong>MÃ¡s credibilidad</strong><span>Se siente mÃ¡s autÃ©ntica.</span></div>
        <div class="pub-cred-point"><strong>MÃ¡s confianza</strong><span>Ayuda a decidir mÃ¡s rÃ¡pido.</span></div>
        <div class="pub-cred-point"><strong>MÃ¡s motivaciÃ³n</strong><span>Reconoce el esfuerzo real.</span></div>
      </div>
    </div>
    <div class="pub-cred-card">
      <h3>CÃ³mo funciona</h3>
      <p>EscribÃ­s tu reseÃ±a, elegÃ­s el monto y pagÃ¡s. Cuando el pago se aprueba, se publica automÃ¡ticamente.</p>
    </div>`;
  document.getElementById('pubTrustGrid').innerHTML = `
    <div class="pub-trust-item"><strong>Claro</strong><span>La acciÃ³n principal estÃ¡ siempre visible.</span></div>
    <div class="pub-trust-item"><strong>Seguro</strong><span>Pago protegido con Mercado Pago.</span></div>
    <div class="pub-trust-item"><strong>Profesional</strong><span>Las reseÃ±as ordenan la reputaciÃ³n del perfil.</span></div>`;
  document.getElementById('pubCtaPanel').innerHTML = `
    <div class="pub-cta-main">
      <h3>ReconocÃ© una buena experiencia.</h3>
      <p>Tu reseÃ±a suma confianza y valor al perfil.</p>
      <div class="pub-cta-points">
        <span class="pub-cta-chip">Menos de 1 minuto</span>
        <span class="pub-cta-chip">AnÃ³nimo si querÃ©s</span>
    ...
    <div class="pub-cta-side">
      <h3>Contacto</h3>
      <p>${hasPhone ? 'PodÃ©s contactar al perfil por telÃ©fono o WhatsApp.' : 'Este perfil todavÃ­a no agregÃ³ un telÃ©fono pÃºblico.'}</p>
      <div class="pub-contact-list">
        <a class="pub-contact-btn${hasPhone ? '' : ' disabled'}" href="${hasPhone ? phoneLink(phone) : '#'}">${hasPhone ? `Llamar: ${phone}` : 'TelÃ©fono no disponible'}</a>
  ...
  document.getElementById('pubStats').innerHTML = `
    <div class="stat-cell"><span class="stat-v">${reviews.length}</span><div class="stat-l">ReseÃ±as publicadas</div></div>
  ...
  document.getElementById('revCount').textContent = reviews.length + ' reseÃ±as Â· ordenadas por recientes';
  ...
  document.getElementById('pubReviews').innerHTML = reviews.length
    ...
    : `<div class="rev-card"><p class="rev-text">TodavÃ­a no hay reseÃ±as publicadas. La primera puede ser la tuya.</p><button class="btn btn-amber btn-sm" onclick="nav('form')">Escribir la primera reseÃ±a</button></div>`;
  renderPublicRewards(rewardItems);
  renderPublicMediaPreview(mediaItems);
  renderMediaVault();
  return;
  document.getElementById('pubStats').innerHTML = `
    <div class="stat-cell"><span class="stat-v">${STATE.reviews.length}</span><div class="stat-l">ReseÃ±as</div></div>
  ...
  document.getElementById('revCount').textContent = STATE.reviews.length + ' reseÃ±as Â· ordenadas por recientes';
...
function revCardHTML(r, isDash) {
  const replyBtn = isDash && !r.reply
    ...
    : (isDash && r.reply ? `<button class="d-rev-btn" style="color:var(--green)" disabled>âœ“ Respondida</button>` : '');
    ...
    <div class="d-rev-item">
      ...
      <div class="d-rev-body">
        <div class="d-rev-top">
          ...
          <span class="badge badge-amber">âœ¦ $${r.amount.toLocaleString('es-AR')}</span>
  ...
  return `
    <div class="rev-card">
      <div class="rev-header">
        ...
        <div class="rev-amount">âœ¦ $${r.amount.toLocaleString('es-AR')}</div>
      </div>
...
function submitReply() {
  if (sb && STATE.user.id) {
    ...
    (async () => {
      ...
      toast('Respuesta publicada âœ“','success');
      closeModal();
      renderDashboard();
      renderProfile();
    })();
...
function renderDashboard() {
  ...
  if (ds) ds.textContent = STATE.user.role + ' Â· ' + STATE.user.city;
  ...
  if (af) af.innerHTML = STATE.reviews.slice(0,4).map(r => `
    <div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--border);">
      ...
      <span class="badge badge-amber" style="font-size:11px">âœ¦$${(r.amount/1000).toFixed(r.amount%1000?1:0)}k</span>
    </div>`).join('');
  ...
  if (ph) ph.innerHTML = STATE.reviews.slice(0, 10).map(p => `
    <div class="pay-row">
      <div class="pay-row-left">
        ...
        <div class="pay-row-info"><div class="pay-row-name">${p.name}</div><div class="pay-row-date">${p.date} Ã‚Â· MercadoPago</div></div>
      </div>
...
function saveProfile() {
  if (sb && STATE.user.id) {
    (async () => {
      ...
      toast('Perfil actualizado âœ“','success');
      updateNav();
      renderDashboard();
      renderProfile();
    })();
...
async function saveAccountSettings() {
  if (!sb || !STATE.user.id) {
    toast('IniciÃ¡ sesiÃ³n para guardar tu cuenta','error');
    return;
  }
  ...
  renderDashboard();
  renderProfile();
  toast('Cuenta actualizada âœ“','success');
}
...
function updateMpStatusBadge() {
  ...
  if (!pk) {
    ...
    badge.textContent = 'âš¡ Sin configurar';
  } else if (STATE.mpConfig.mode === 'production' || CONFIG.mpMode === 'production') {
    badge.className = 'mp-status connected';
    badge.textContent = 'âœ“ Activo en producciÃ³n';
  } else {
    badge.className = 'mp-status configured';
    badge.textContent = 'ðŸ§ª Configurado en sandbox';
  }
}

async function savePaymentSettings() {
  if (!sb || !STATE.user.id) {
    toast('Primero iniciÃ¡ sesiÃ³n para guardar tus datos de cobro','error');
    return;
  }
  ...
  toast('Datos de cobro guardados âœ“','success');
  renderProfile();
}
...
async function testMpConnection() {
  const pk = getMpPublicKey();
  if (!pk) { toast('Primero configurÃ¡ la Public Key','error'); return; }
  toast('Probando conexiÃ³n...','info');
  setTimeout(() => {
    if (pk.startsWith('APP_USR-') || pk.startsWith('TEST-')) {
      toast('âœ“ Public Key vÃ¡lida Â· ConexiÃ³n OK','success');
    } else {
      toast('Public Key con formato invÃ¡lido','error');
    }
  }, 1000);
}
...
function copyWebhook() {
  ...
  if (navigator.clipboard) {
    navigator.clipboard.writeText(url).then(() => toast('URL copiada âœ“','success'));
  } else {
    toast('URL: ' + url,'info');
  }
}

// Modal MP Config (acceso rÃ¡pido)
  ...
  grid.innerHTML = filtered.map(p => `
    <div class="p-card" onclick="openPublicProfile('${p.slug || ''}')">
      <div class="p-card-top">
        ...
        <div><div class="p-card-name">${p.name}</div><div class="p-card-role">${p.role} Â· ${p.city}</div></div>
      </div>
...
function doShare() {
  ...
  if (navigator.share) {
    navigator.share({ title:'Recomendapp', text:'MirÃ¡ el perfil de ' + targetName, url });
    return;
  }
  if (navigator.share) {
    navigator.share({ title:'Recomendapp', text:'MirÃ¡ el perfil de '+STATE.user.name, url });
  } else if (navigator.clipboard) {
    navigator.clipboard.writeText(url).then(() => toast('Enlace copiado âœ“','success'));
  } else {
...
function buildMarquee() {
  ...
  const html = [...items,...items].map(i => `<div class="m-item"><span class="star">âœ¦</span>${i}</div>`).join('');
  ...
  if (status) {
    if ((params.get('media') === '1' || unlockId) && pendingMedia) {
      ...
      if (confirmedUnlock?.payment_status === 'approved') {
        ...
        setTimeout(() => toast('El pago del contenido no fue aprobado. IntentÃ¡ de nuevo.','error'), 300);
      }
