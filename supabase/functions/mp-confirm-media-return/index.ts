...
serve(async (req) => {
  ...
  try {
    ...
    const { unlockId, paymentId, merchantOrderId, profileSlug } = await req.json();
    if (!unlockId || (!paymentId && !merchantOrderId) || !profileSlug) {
      return Response.json({ error: "Faltan unlockId, paymentId/merchantOrderId o profileSlug" }, { status: 400, headers: corsHeaders });
    }
    ...
    const resolvedUnlockId = payment.external_reference || payment.metadata?.unlock_id || unlockId;

    await supabase.rpc("mark_media_unlock_paid", {
      p_unlock_id: resolvedUnlockId,
      p_status: status,
      p_mp_payment_id: resolvedPaymentId,
    });

    return Response.json(
      { ok: true, unlock_id: resolvedUnlockId, payment_status: status, payment_id: resolvedPaymentId },
